...
async function main() {
  console.log('Populando banco com planos iniciais...')
  ...
  for (const p of plans) {
    ...
    if (!existing) {
      ...
      console.log(`✓ Plano "${p.name}" criado.`)
    } else {
      console.log(`→ Plano "${p.name}" já existe, pulando.`)
    }
  }
  ...
  for (const a of achievements) {
    ...
    if (!existing) {
      ...
      console.log(`✓ Conquista "${a.name}" criada.`)
    }
  }

  // Adicionar item especial à loja (Poção de XP) se não existir
  const potion = await prisma.reward.findFirst({ where: { name: 'Poção de XP' } })
  if (!potion) {
    // Como o Reward exige um userId (é um sistema de recompensas personalizadas), 
    // no seed não temos um user fácil. Mas os planos criam hábitos. 
    // Deixaremos para o usuário criar sua poção ou adicionaremos a todos os usuários depois.
  }
